...
func writeSceneWallpaper(input inputDir, project Project) error {
	...
	if project.File != "scene.json" {
		return fmt.Errorf("unsupported scene file %q", project.File)
	}

	env.Assets = os.Getenv("WPE_COMPILE_ASSETS")
	if env.Assets == "" {
		return fmt.Errorf("WPE_COMPILE_ASSETS is not set")
	}

	pkgFile, err := os.ReadFile(input.PackagePath)
	if err != nil {
		return fmt.Errorf("open pkg failed: %w", err)
	}
	state.PKGMap, err = extractPkg(pkgFile)
	if err != nil {
		return fmt.Errorf("extract pkg failed: %w", err)
	}

	state.Scene, err = ParseScene(&state.PKGMap)
	if err != nil {
		return fmt.Errorf("parse scene.json failed: %w", err)
	}
	...
	if args.SkipEffects != "" {
		if err := applySkipEffects(args.SkipEffects); err != nil {
			return fmt.Errorf("invalid --skip-effects: %w", err)
		}
	}
	if args.SkipObjects != "" {
		if err := applySkipObjects(args.SkipObjects); err != nil {
			return fmt.Errorf("invalid --skip-objects: %w", err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("encoding scene data failed: %w", err)
	}
